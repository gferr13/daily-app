            ...
            <div key={store.id} onClick={() => setSelected(store)}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: '#555', fontSize: 12 }}>
                    {store.deals.length > 0
                      ? `${store.deals.length} deal${store.deals.length > 1 ? 's' : ''} active`
                      : 'No active deals'}
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  ...
                  {store.deals.length > 0 && (
                    <span style={{ background: '#1a3a1a', color: '#4caf50', fontSize: 10, fontWeight: 700, borderRadius: 6, padding: '2px 7px' }}>
                      DEALS
                    </span>
                  )}
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {selected.deals.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 20px', color: '#333', fontSize: 13 }}>
            ...
            <div style={{ color: '#2a2a2a', fontSize: 11, marginTop: 6 }}>Check back soon — deals refresh every 3 hours</div>
          </div>
        )}
